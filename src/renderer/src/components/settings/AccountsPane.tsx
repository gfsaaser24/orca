...
export function AccountsPane({
  ...
  const { state: observedTeamclaudeState } = useTeamclaude()
  const effectiveTeamclaudeState =
    teamclaudeState === undefined ? observedTeamclaudeState : teamclaudeState
  const teamclaudeConnected =
    effectiveTeamclaudeState?.lifecycle === 'adopted' ||
    effectiveTeamclaudeState?.lifecycle === 'adopted-degraded' ||
    effectiveTeamclaudeState?.lifecycle === 'owned'
      ...
      <section key="claude-accounts" id="accounts-claude" className="space-y-4 scroll-mt-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            {teamclaudeConnected
              ? translate(
                  'auto.components.settings.AccountsPane.teamclaudeManagedIntro',
                  'TeamClaude is managing Claude routing and account selection.'
                )
              : translate(
        ...
        <SearchableSetting
          ...
          description={
            teamclaudeConnected
              ? translate(
                  'auto.components.settings.AccountsPane.teamclaudeManagedDescription',
                  'TeamClaude owns Claude routing and account selection while connected.'
                )
              : translate(
        ...
        >
          {teamclaudeConnected ? (
            <div className="flex items-center justify-between gap-3 rounded-md border border-border/70 bg-accent/8 px-3 py-3 max-sm:flex-col max-sm:items-start">
              <div className="min-w-0 space-y-1">
                <div className="flex items-center gap-2">
                  <Badge variant="outline">
                    {translate(
                      'auto.components.settings.AccountsPane.teamclaudeManagedBadge',
                      'Managed by TeamClaude'
                    )}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  {translate(
                    'auto.components.settings.AccountsPane.teamclaudeManagedCopy',
                    'Add, sign in, re-authenticate, and switch Claude accounts from the TeamClaude cockpit.'
                  )}
                </p>
              </div>
              <Button
                ...
                onClick={openTeamclaudeCockpit}
              >
                <ExternalLink className="size-3" />
                {translate(
                  'auto.components.settings.AccountsPane.openTeamclaudeCockpit',
